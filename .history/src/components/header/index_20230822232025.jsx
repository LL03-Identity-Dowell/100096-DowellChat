      ...
      <dialog id="my_modal_3" className="modal">
        <form method="dialog" className="modal-box">
          ...
          <span className="">
            ...
            <p key={session_id} className="bg-gray-200 mb-2 px-2 py-4 rounded font-bold">
              {session_id}
            </p>
          )}
          </span>
         <div className={`mt-5  ${active ?  null : "flex justify-end" }`}>
          {
            active ? (
              <form>
                ...
                <button className="btn btn-md ml-2 btn-primary">Add</button>
              </form>
            ) :  <a className="p-4 rounded-lg text-white text-center bg-primary" onClick={()=>setActive(!active)}>Add</a>
          }
         </div>
        </form>
      </dialog>
