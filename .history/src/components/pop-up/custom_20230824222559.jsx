...
const CustomPopUp = ({isLoading,showPopup,rooms,title},) => {
  ...
  return (
      <div className={`flex overflow-auto fixed flex-col gap-10 justify-center items-center outline-1 border-none bg-gray-100 rounded-lg top-1/2 left-1/2 h-96 ${next ? "w-1/4" : 'max-w-96'} shadow-lg -translate-x-1/2 -translate-y-1/2`}>
       {
        next ? <>
         <h1 className='font-bold text-3xl text-gray-600'>Product  name</h1>
        <form action="">
          <input type="text"  placeholder='product name...' value={product}  onChange={(event)=>setProduct(event.target.value)}className='p-2 rounded-md'/>
          ...
          <h1>{title}</h1>
            ...
            <div className="">
                ...
                <input type="text" placeholder="Type here" value={value} onChange={(e)=>setValue(e.target.value)} className="p-2 rounded-md" />
                   {value ? <button className='bg-blue-600 text-white py-2 px-4 rounded-r-md' onClick={handleNextPopUP}>Next</button> : null}
                </form>
            </div>
